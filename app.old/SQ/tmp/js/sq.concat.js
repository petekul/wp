...
//            $state.go(routingService.getNextState());
    ...
    SQService.loadSQConfig().then(function(response){
        ...
        //  ES5
        $scope.midConfig = {};
        $scope.postcodeConfig ={};
        $scope.contactConfig = {};

        for(var i=0;i<SQConfig.length;i++){
            if(SQConfig[i].questionType == "MID"){
                $scope.midConfig = SQConfig[i];
            }
            if(SQConfig[i].questionType == "POSTCODE"){
                $scope.postcodeConfig = SQConfig[i];
            }
            if(SQConfig[i].questionType == "LASTNAME"){
                $scope.contactConfig = SQConfig[i];
            }
        }
...
//                $state.go(routingService.getNextState(), {"SQtoken":response.data.sqtoken});
    ...
    function checkSubmitDisabled(){
        ...
        //ES5
        if(disabledFlag)
            $("#btnSubmitSQ").prop("disabled", true);
        else
            $("#btnSubmitSQ").prop("disabled", false);
    }
    ...
    .factory("SQService", ["$http", "$q", "urlService", function($http, $q, urlService) {
        ...
        return {
            getSecurityQuestions: function(directive, backUrl){
                return $http.get(urlService.getURL(scm + "SQ"), {
                    headers: {
                        directive: directive,
                        "backUrl": backUrl
                    }
